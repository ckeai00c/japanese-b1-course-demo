...
这是一个无构建依赖的移动端日语课程模板。当前仓库自带“二次元游戏黑话”示例；后续制作漫画、交通、影视等主题时，只替换课程配置和素材，不需要复制或重写交互结构。
...
框架统一处理：自动播放、常速/慢速播放、chunk 点击音频、答对/答错音效、错误标红回弹、正确标绿、完成后复现原内容、中文提示、汉字注音、句子标点固定显示和口语即时判题。用户在任意口语题点击“不做口语题”后，本轮全部口语题都会切换成各自句子的 chunk 拼写题。

正确 chunk 会立即变绿，但引擎会等待该 chunk 音频自然播放完毕后再显示下一组或完成页。切换到口语替代题时会停止当前整句音频，替代题开始时保持静音。
...
2. 修改 `course.config.js` 中的 `meta`、`theme`、`readings`、`vocabulary` 和 `sentences`。
